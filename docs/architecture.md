...
- `src/server/webhooks`: inbound provider events, and outbound delivery to customer endpoints. Outbound uses no Queue — a delivery row plus `waitUntil`, retried by the existing cron — because Queue operations are capped on the Free plan and reserved for mail. `destination.ts` validates user-supplied URLs at creation and again immediately before each send; redirects are not followed and link-local metadata addresses are refused unconditionally. Payloads carry ids and statuses, never message text. Only generic endpoints are signed; Slack and Telegram authenticate through their own secret URL or bot token.
- `src/server/mcp`: a read-only Model Context Protocol server at `/api/mcp`, speaking JSON-RPC over HTTP. The envelope is hand-rolled rather than taken from the MCP SDK, to keep the Worker bundle well inside its 3 MB compressed ceiling. It reuses the API-key middleware verbatim, including the live-role recheck, and derives the workspace from the key — no tool accepts an organization id. Five read tools; no write tool exists.
- `src/server/api-keys` and `src/server/auth/api-key.ts`: scoped keys for `/api/v1`. Only a SHA-256 hash is stored, uniquely indexed, so authentication is one lookup. Authorization re-reads the creating member's live role on every request and intersects it with the key's stored scopes, so a demotion takes effect immediately without re-issuing anything. Surfaces with no scope in the vocabulary are unreachable with a key. `/api/*` remains session-only, which is what keeps the CSRF boundary coherent.
- `src/server/csat`: satisfaction ratings. Rating links carry `<ticketId>.<rating>.<signature>`, signed with `SESSION_PEPPER`; nothing token-shaped is stored, so reading the database does not let anyone forge a link, and the rating is inside the signed payload so it cannot be edited upward. The public rating endpoints are the only unauthenticated mutation in the product: they verify the signature before touching the database, are rate limited, and answer every failure identically so they never reveal whether a ticket exists.
...
- `src/server/backups`: workspace export. Every tenant-scoped table is written to R2 as NDJSON under a reserved `_backups/` prefix in the attachments bucket, keyset-paginated by `rowid` because several exported tables have composite primary keys. R2 objects cannot be appended to, so each slice is its own numbered object and the download route concatenates a table's chunks in order; memory stays flat whatever the workspace's size. Credentials never reach an export: `sessions` and `password_reset_tokens` are not exported at all, and password hashes, API-key hashes, webhook secrets and bot tokens — plus the URL of a Slack or Telegram endpoint, where the URL is itself the credential — are redacted per row before anything is written. Export only: there is no restore path, because a half-applied one is worse than none.
