...
AI assistance uses Cloudflare Workers AI by default. `wrangler.jsonc` declares the `AI` binding, so `wrangler deploy` provisions it and no credential is required; set `WORKERS_AI_MODEL` to override the text model (default `@cf/meta/llama-4-scout-17b-16e-instruct`) and `AI_GATEWAY_ID` to route the calls through an AI Gateway. To use OpenAI instead, remove the `ai` block from `wrangler.jsonc` and add `OPENAI_API_KEY` (and optionally `OPENAI_MODEL`, default `gpt-4o-mini`) as Worker secrets; the binding wins whenever both are present.
