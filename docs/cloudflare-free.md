...
| Workers AI     | Yes, 10,000 neurons/day                | A shipped binding could spend the daily allowance without the workspace asking for it                    | Binding ships commented out (opt-in); once enabled it stays inert until an admin enables AI in Settings; bounded output, chunked translation; over the allowance calls fail as `503` and drafts are untouched |
...
- Outbound webhooks deliberately spend **no Queue operations**. A delivery is a D1 row attempted on `waitUntil`, with retries claimed by one more bounded statement in the existing Cron batch (twenty per run) and delivered rows swept after seven days by another. A workspace is capped at ten endpoints, and an endpoint that fails ten times in a row disables itself, so a dead consumer cannot consume the retry budget indefinitely. The free-plan test asserts both the bound and that the queue producers are never called.
- The MCP endpoint is request-driven, not scheduled, and adds nothing to the Cron budget. Every tool caps its result set at fifty rows and truncates long message bodies and article text, so a single call cannot turn into an unbounded read. The Worker bundle including MCP is roughly 181 KB compressed, well inside the 3 MB limit.
- Waking expired snoozes is one more bounded statement in the same Cron batch, also limited to twenty tickets per run. A workspace with more than twenty snoozes expiring in one five-minute window sees the rest wake on the following runs. Waking on a customer's reply is not bounded and not deferred: it happens in the same write as the incoming message.
- SLA state is maintained by two extra statements in the existing five-minute Cron batch, each bounded to twenty tickets, and no second Cron trigger is added. A workspace whose backlog of un-answered tickets grows faster than forty promotions per five minutes will lag behind; the Overdue queue is then late rather than wrong. The "due soon" threshold is derived in SQL from the stored timestamps rather than by running the business-hours calculation per row, which would not fit the CPU budget — the warning is approximate, the breach itself is exact.
