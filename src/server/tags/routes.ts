...
const tagInput = z.object({ name: z.string().trim().min(1).max(40).transform((value) => value.toLowerCase()), color: z.enum(["slate", "blue", "green", "amber", "red", "violet", "pink"]).default("slate") });
...
tagRoutes.post("/", zValidator("json", tagInput), async (context) => {
  ...
  const id = newId("tag");
  try {
    await createDb(context.env.DB).insert(tags).values({ id, organizationId: tenant.organizationId, ...input });
  } catch (error) {
    if (String(error).includes("UNIQUE")) throw new HttpError(409, "tag_exists", "A tag with this name already exists.");
    throw error;
  }
