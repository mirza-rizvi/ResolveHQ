...
ticketRoutes.get("/", async (context) => {
  ...
  const rows = await db
    ...
    .leftJoin(
      ticketReadStates,
      and(
        eq(ticketReadStates.ticketId, tickets.id),
        eq(ticketReadStates.organizationId, tenant.organizationId),
        eq(ticketReadStates.userId, tenant.userId),
      ),
    )
    .where(
      and(
        eq(tickets.organizationId, tenant.organizationId),
        status && ticketStatuses.includes(status as (typeof ticketStatuses)[number])
          ? eq(tickets.status, status as (typeof ticketStatuses)[number])
          : undefined,
        priority && ticketPriorities.includes(priority as (typeof ticketPriorities)[number])
          ? eq(tickets.priority, priority as (typeof ticketPriorities)[number])
          : undefined,
        assignee === "me"
          ? eq(tickets.assignedUserId, tenant.userId)
          : assignee === "unassigned"
        ...
        cursor
          ? or(
              lt(tickets.updatedAt, new Date(cursor.updatedAt)),
              and(eq(tickets.updatedAt, new Date(cursor.updatedAt)), lt(tickets.id, cursor.id)),
            )
          : undefined,
      ),
    )
  ...
  const nextCursor =
    hasMore && last ? encodeCursor({ updatedAt: new Date(last.updatedAt).getTime(), id: last.id }) : null;
  const tagMap = new Map<string, Array<{ id: string; name: string; color: string }>>();
  if (items.length) {
    const placeholders = items.map(() => "?").join(",");
    const tagRows = await context.env.DB.prepare(
      `SELECT tt.ticket_id AS ticketId, g.id, g.name, g.color FROM ticket_tags tt JOIN tags g ON g.id = tt.tag_id AND g.organization_id = tt.organization_id WHERE tt.organization_id = ? AND tt.ticket_id IN (${placeholders}) ORDER BY g.name`,
    )
      .bind(tenant.organizationId, ...items.map((item) => item.id))
      .all<{ ticketId: string; id: string; name: string; color: string }>();
    for (const tag of tagRows.results)
      tagMap.set(tag.ticketId, [...(tagMap.get(tag.ticketId) ?? []), { id: tag.id, name: tag.name, color: tag.color }]);
  }
...
ticketRoutes.post("/", validate("json", createTicketInput), async (context) => {
  ...
  const [customer] = await db
    ...
    .from(customers)
    .where(and(eq(customers.id, input.customerId), eq(customers.organizationId, tenant.organizationId)))
    .limit(1);
  ...
  const defaultInbox = await db
    .select({ id: inboxes.id })
    .from(inboxes)
    .where(and(eq(inboxes.organizationId, tenant.organizationId), isNull(inboxes.disabledAt)))
    .orderBy(desc(inboxes.isDefault), asc(inboxes.createdAt))
    .limit(1)
    .then((rows) => rows[0]);
  if (!defaultInbox)
    throw new HttpError(409, "no_inbox", "Add a support inbox in Settings before starting conversations.");
  ...
  const numberRow = await context.env.DB.prepare(
    ...
    .bind(Date.now(), tenant.organizationId)
    .first<{ number: number }>();
  ...
  await db.batch([
    ...
    db
      ...
      .where(and(eq(customers.id, customer.id), eq(customers.organizationId, tenant.organizationId))),
  ]);
...
ticketRoutes.patch("/:id", validate("json", updateTicketInput), async (context) => {
  ...
  const ticket = await applyTicketUpdate(context.env, tenant, context.req.param("id"), changes, {
...
ticketRoutes.post("/:id/messages", validate("json", messageInput), async (context) => {
  const tenant = context.get("tenant");
  if (!(await context.env.WRITE_RATE_LIMIT.limit({ key: `messages:${tenant.userId}` })).success)
    throw new HttpError(429, "rate_limited", "Slow down and try again in a moment.");
  const input = context.req.valid("json");
  const db = createDb(context.env.DB);
  const [ticket] = await db
    .select({ id: tickets.id, customerId: tickets.customerId, status: tickets.status })
    .from(tickets)
    .where(and(eq(tickets.id, context.req.param("id")), eq(tickets.organizationId, tenant.organizationId)))
    .limit(1);
  if (!ticket) throw new HttpError(404, "ticket_not_found", "Ticket not found.");
  ...
  const findExisting = clientMessageId
    ? () =>
        db
          ...
          .from(messages)
          .where(and(eq(messages.organizationId, tenant.organizationId), eq(messages.clientMessageId, clientMessageId)))
          .limit(1)
          .then((rows) => rows[0])
    : null;
  ...
  const values: typeof messages.$inferInsert = {
    ...
    kind: input.kind,
    bodyText: input.body,
    bodyHtml: input.kind === "message" && input.bodyHtml ? sanitizeHtml(input.bodyHtml) || null : null,
    normalizedSearch: normalizeSearch(input.body),
    clientMessageId: input.clientMessageId,
    deliveryStatus: input.kind === "message" ? "queued" : "received",
  };
    ...
    if (claimed.meta.changes !== attachmentIds.length) {
      await db.batch([
        db
          .update(attachments)
          .set({ messageId: null })
          .where(and(eq(attachments.organizationId, tenant.organizationId), eq(attachments.messageId, id))),
  ...
  const updateTicket = db
    .update(tickets)
    .set({
      updatedAt: now,
      lastReplyAt: input.kind === "message" ? now : undefined,
      lastAgentReplyAt: input.kind === "message" ? now : undefined,
  ...
  if (handsOffToCustomer) {
    await context.env.DB.prepare(
      "UPDATE tickets SET status = 'waiting_customer', waiting_since = ? WHERE organization_id = ? AND id = ?",
    )
      .bind(now.getTime(), tenant.organizationId, ticket.id)
      .run();
  }
  await refreshTicketSearch(context.env.DB, tenant.organizationId, ticket.id);
  await recordActivity(db, tenant, {
    ticketId: ticket.id,
    eventType: input.kind === "internal_note" ? "ticket.note_added" : "ticket.agent_replied",
    entityType: "message",
    entityId: id,
  });
  if (handsOffToCustomer) {
...
ticketRoutes.post("/:id/tags", validate("json", z.object({ tagId: z.string().min(1) })), async (context) => {
  ...
  const [tag] = await db
    .select({ id: tags.id })
    .from(tags)
    .where(and(eq(tags.id, tagId), eq(tags.organizationId, tenant.organizationId)))
    .limit(1);
  if (!tag) throw new HttpError(404, "tag_not_found", "Tag not found.");
  await db
    .insert(ticketTags)
    .values({ organizationId: tenant.organizationId, ticketId: context.req.param("id"), tagId })
    .onConflictDoNothing();
  await refreshTicketSearch(context.env.DB, tenant.organizationId, context.req.param("id"));
  await recordActivity(db, tenant, {
    ticketId: context.req.param("id"),
...
ticketRoutes.delete("/:id/tags/:tagId", async (context) => {
  ...
  await recordActivity(db, tenant, {
    ...
    entityType: "tag",
    entityId: context.req.param("tagId"),
  });
...
function decodeCursor(value?: string) {
  ...
  try {
    const decoded = JSON.parse(atob(value.replaceAll("-", "+").replaceAll("_", "/"))) as {
      updatedAt?: unknown;
      id?: unknown;
    };
    return typeof decoded.updatedAt === "number" && typeof decoded.id === "string"
