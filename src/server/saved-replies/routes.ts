...
savedReplyRoutes.get("/", async (context) => {
  ...
  const rows = await createDb(context.env.DB)
    .select()
    .from(savedReplies)
    .where(eq(savedReplies.organizationId, tenant.organizationId))
    .orderBy(asc(savedReplies.category), asc(savedReplies.name));
  return context.json({ savedReplies: rows });
});
savedReplyRoutes.post("/", validate("json", inputSchema), async (context) => {
  const tenant = context.get("tenant");
  const input = context.req.valid("json");
  const id = newId("rpl");
  await createDb(context.env.DB)
    .insert(savedReplies)
    .values({ id, organizationId: tenant.organizationId, createdByUserId: tenant.userId, ...input });
  return context.json({ savedReply: { id, ...input } }, 201);
});
savedReplyRoutes.patch("/:id", validate("json", inputSchema.partial()), async (context) => {
  const tenant = context.get("tenant");
  await createDb(context.env.DB)
    .update(savedReplies)
