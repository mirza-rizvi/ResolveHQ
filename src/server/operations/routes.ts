...
operationRoutes.get("/dashboard", async (context) => {
  ...
  const metrics = await context.env.DB.prepare(
    ...
    .bind(startOfToday.getTime(), tenant.organizationId)
...
operationRoutes.get("/notifications", async (context) => {
  ...
  const rows = await createDb(context.env.DB)
    .select()
    .from(notifications)
    .where(and(eq(notifications.organizationId, tenant.organizationId), eq(notifications.userId, tenant.userId)))
...
operationRoutes.get("/tickets/:ticketId/draft", async (context) => {
  ...
  const draft = await createDb(context.env.DB)
    .select()
    .from(ticketDrafts)
    .where(
      and(
        eq(ticketDrafts.organizationId, tenant.organizationId),
        eq(ticketDrafts.ticketId, context.req.param("ticketId")),
        eq(ticketDrafts.userId, tenant.userId),
      ),
    )
    .limit(1)
...
operationRoutes.put("/tickets/:ticketId/draft", validate("json", draftInput), async (context) => {
  const tenant = context.get("tenant");
  if (!(await context.env.WRITE_RATE_LIMIT.limit({ key: `drafts:${tenant.userId}` })).success)
  ...
  const current = await context.env.DB.prepare(
    "SELECT revision FROM ticket_drafts WHERE organization_id = ? AND ticket_id = ? AND user_id = ?",
  )
    .bind(tenant.organizationId, ticketId, tenant.userId)
    .first<{ revision: number }>();
  if (current && input.revision < current.revision)
    throw new HttpError(409, "draft_revision_conflict", "A newer draft already exists.");
  ...
  await createDb(context.env.DB)
    ...
    .onConflictDoUpdate({
      ...
      set: { body: input.body, kind: input.kind, revision: nextRevision, updatedAt: new Date() },
    });
...
operationRoutes.post("/tickets/:ticketId/read", async (context) => {
  ...
  await createDb(context.env.DB)
    ...
    .values({ organizationId: tenant.organizationId, ticketId, userId: tenant.userId, lastReadAt: new Date() })
    .onConflictDoUpdate({
...
operationRoutes.get("/views", async (context) => {
  ...
  const rows = await createDb(context.env.DB)
    ...
    .where(
      and(
        eq(savedViews.organizationId, tenant.organizationId),
        or(eq(savedViews.visibility, "shared"), eq(savedViews.ownerUserId, tenant.userId)),
      ),
    )
...
operationRoutes.post(
  ...
  async (context) => {
    const tenant = context.get("tenant");
    const input = context.req.valid("json");
    if (input.visibility === "shared" && tenant.role === "agent")
      throw new HttpError(403, "forbidden", "Only admins can create shared views.");
    ...
    const existing = await context.env.DB.prepare(
      ...
      .bind(tenant.organizationId, tenant.userId, input.name, normalizedFilters)
      .first<{ id: string; name: string; visibility: "personal" | "shared"; filters: string }>();
    if (existing) {
      return context.json(
        {
          view: {
            ...
            filters: JSON.parse(existing.filters),
          },
        },
        200,
      );
...
operationRoutes.delete("/views/:id", async (context) => {
  const tenant = context.get("tenant");
  const [view] = await createDb(context.env.DB)
    .select({ id: savedViews.id, ownerUserId: savedViews.ownerUserId })
    .from(savedViews)
    .where(and(eq(savedViews.id, context.req.param("id")), eq(savedViews.organizationId, tenant.organizationId)))
    .limit(1);
  if (!view) throw new HttpError(404, "view_not_found", "Saved view not found.");
  if (view.ownerUserId !== tenant.userId && roleRank[tenant.role] < roleRank.admin)
    throw new HttpError(403, "forbidden", "You can only delete your own saved views.");
  await createDb(context.env.DB)
    .delete(savedViews)
    .where(and(eq(savedViews.id, context.req.param("id")), eq(savedViews.organizationId, tenant.organizationId)));
  return context.body(null, 204);
...
operationRoutes.get("/teams", async (context) => {
  ...
  const rows = await context.env.DB.prepare(
    "SELECT t.id, t.name, count(tm.user_id) AS memberCount FROM teams t LEFT JOIN team_members tm ON tm.team_id = t.id AND tm.organization_id = t.organization_id WHERE t.organization_id = ? GROUP BY t.id ORDER BY t.name",
  )
    .bind(tenant.organizationId)
    .all();
...
operationRoutes.post(
  ...
  validate(
    "json",
    z.object({ name: z.string().trim().min(1).max(80), userIds: z.array(z.string()).max(50).default([]) }),
  ),
  async (context) => {
    const tenant = context.get("tenant");
    const input = context.req.valid("json");
...
operationRoutes.get("/dev-mail", requireRole("admin"), async (context) => {
  if (context.env.DEV_MAIL_MODE !== "capture" || context.env.RESEND_API_KEY)
    throw new HttpError(404, "not_found", "Mail capture is not enabled.");
  const tenant = context.get("tenant");
  const me = await context.env.DB.prepare("SELECT email FROM users WHERE id = ?")
    .bind(tenant.userId)
    .first<{ email: string }>();
  const rows = await context.env.DB.prepare(
    ...
    .bind(tenant.organizationId, me?.email ?? "")
    .all();
