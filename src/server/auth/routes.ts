...
authRoutes.post("/signup", validate("json", signupInput), async (context) => {
  ...
  const rate = await context.env.AUTH_RATE_LIMIT.limit({ key: `signup:${ip}` });
  ...
  try {
    ...
    if (failure.includes("UNIQUE")) {
      if (input.supportEmail && (failure.includes("inboxes") || failure.includes("email_address")))
  ...
  return context.json(
    {
      ...
      organization: { id: organizationId, name: input.organizationName, slug: input.organizationSlug },
      role: "owner",
      csrfToken,
    },
    201,
...
authRoutes.post("/login", validate("json", credentials), async (context) => {
  ...
  const [result] = await db
    ...
    .innerJoin(
      organizationMemberships,
      and(eq(organizationMemberships.userId, users.id), isNull(organizationMemberships.disabledAt)),
    )
...
authRoutes.post(
  ...
  async (context) => {
    ...
    const [invitation] = await db
      ...
      .where(and(eq(organizationInvitations.tokenHash, tokenHash), isNull(organizationInvitations.acceptedAt)))
      .limit(1);
    if (!invitation || invitation.expiresAt <= new Date())
      throw new HttpError(404, "invitation_invalid", "This invitation is invalid or has expired.");
    ...
    if (tenant) {
      ...
      await db.batch([
        db
          ...
          .onConflictDoUpdate({
            ...
            set: { role: invitation.role, disabledAt: null },
          }),
        db
      ...
      await db
        .update(sessions)
        .set({ organizationId: invitation.organizationId, lastSeenAt: now })
        .where(
          and(
            eq(sessions.tokenHash, await sha256(`${token}.${context.env.SESSION_PEPPER}`)),
            eq(sessions.userId, tenant.userId),
          ),
        );
    ...
    if (!input.name || !input.password)
      throw new HttpError(400, "validation_error", "name and password are required to create your account.");
    const userId = newId("usr");
    const now = new Date();
    await db.batch([
    ...
    return context.json(
      {
        user: { id: userId, email: invitation.email, name: input.name },
        organizationId: invitation.organizationId,
        role: invitation.role,
        csrfToken,
      },
...
authRoutes.post(
  ...
  async (context) => {
    const ip = context.req.header("cf-connecting-ip") ?? "local";
    const { email, turnstileToken: token } = context.req.valid("json");
    const [byIp, byEmail] = await Promise.all([
      context.env.AUTH_RATE_LIMIT.limit({ key: `forgot:ip:${ip}` }),
      context.env.AUTH_RATE_LIMIT.limit({ key: `forgot:email:${email}` }),
    ]);
...
authRoutes.post(
  "/reset-password",
  validate("json", z.object({ token: z.string().min(20).max(200), password: z.string().min(12).max(128) })),
  async (context) => {
    const ip = context.req.header("cf-connecting-ip") ?? "local";
    if (!(await context.env.AUTH_RATE_LIMIT.limit({ key: `reset:${ip}` })).success)
      throw new HttpError(429, "rate_limited", "Too many requests. Try again shortly.");
    const input = context.req.valid("json");
    const db = createDb(context.env.DB);
    const tokenHash = await sha256(`${input.token}.${context.env.SESSION_PEPPER}`);
    ...
    if (!row || row.expiresAt <= new Date())
      throw new HttpError(404, "reset_invalid", "This reset link is invalid or has expired.");
    await db.batch([
      db
        .update(users)
        .set({
...
authRoutes.get("/me", requireAuth, async (context) => {
  ...
  const workspaceRows = await db
    ...
    .innerJoin(organizations, eq(organizations.id, organizationMemberships.organizationId))
    .where(and(eq(organizationMemberships.userId, tenant.userId), isNull(organizationMemberships.disabledAt)));
  return context.json({
    user: { id: result.userId, email: result.email, name: result.name },
...
authRoutes.post(
  "/switch-workspace",
  requireAuth,
  validate("json", z.object({ organizationId: z.string().min(1) })),
  async (context) => {
    const tenant = context.get("tenant");
    const organizationId = context.req.valid("json").organizationId;
    const membership = await createDb(context.env.DB)
      .select({ id: organizations.id })
      .from(organizationMemberships)
      .innerJoin(organizations, eq(organizations.id, organizationMemberships.organizationId))
      .where(
        and(
          eq(organizationMemberships.userId, tenant.userId),
    ...
    await createDb(context.env.DB)
      ...
      .where(and(eq(sessions.tokenHash, tokenHash), eq(sessions.userId, tenant.userId)));
...
authRoutes.get("/sessions", requireAuth, async (context) => {
  ...
  return context.json({
    sessions: rows.map(({ tokenHash, ...session }) => ({ ...session, current: tokenHash === currentHash })),
  });
});
...
authRoutes.delete("/sessions/:id", requireAuth, async (context) => {
  ...
  const result = await createDb(context.env.DB)
    .delete(sessions)
    .where(and(eq(sessions.id, context.req.param("id")), eq(sessions.userId, tenant.userId)));
