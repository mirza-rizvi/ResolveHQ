...
import { inboxes, organizationInvitations, organizationMemberships, organizations, users } from "resolve-server/db/schema";
...
organizationRoutes.get("/settings", async (context) => {
  ...
  const [workspace, inboxRows] = await Promise.all([
    db.select({ id: organizations.id, name: organizations.name, slug: organizations.slug }).from(organizations).where(eq(organizations.id, tenant.organizationId)).limit(1).then((rows) => rows[0]),
    db.select({ id: inboxes.id, name: inboxes.name, emailAddress: inboxes.emailAddress, provider: inboxes.provider, isDefault: inboxes.isDefault, disabledAt: inboxes.disabledAt }).from(inboxes).where(eq(inboxes.organizationId, tenant.organizationId)),
  ]);
...
organizationRoutes.post("/inboxes", requireRole("admin"), validate("json", z.object({ name: z.string().trim().min(1).max(80), emailAddress: z.string().trim().email().max(254).transform((value) => value.toLowerCase()) })), async (context) => {
  ...
  const input = context.req.valid("json");
  const id = newId("inb");
  try { await createDb(context.env.DB).insert(inboxes).values({ id, organizationId: tenant.organizationId, name: input.name, emailAddress: input.emailAddress, provider: "cloudflare_email" }); }
  catch (error) { if (String(error).includes("UNIQUE")) throw new HttpError(409, "inbox_address_exists", "That inbox address is already in use."); throw error; }
...
organizationRoutes.patch("/members/:userId", requireRole("admin"), validate("json", z.object({ role: z.enum(["admin", "agent"]).optional(), disabled: z.boolean().optional() })), async (context) => {
  ...
  const target = await createDb(context.env.DB).select({ role: organizationMemberships.role }).from(organizationMemberships).where(and(eq(organizationMemberships.organizationId, tenant.organizationId), eq(organizationMemberships.userId, targetUserId))).limit(1).then((rows) => rows[0]);
  if (!target) throw new HttpError(404, "member_not_found", "Team member not found.");
  if (target.role === "owner") throw new HttpError(403, "owner_protected", "The workspace owner cannot be changed or disabled.");
