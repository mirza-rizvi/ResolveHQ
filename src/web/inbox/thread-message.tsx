...
export function ThreadMessage({ message, customerName, customerEmail, attachments }: ThreadMessageProps) {
  if (message.authorType === "system") {
    return <article className="thread-entry system">
  ...
  const delivery = message.authorType === "agent" && message.kind === "message" ? deliveryLabels[message.deliveryStatus] : undefined;
  return <article className={`thread-entry ${message.kind === "internal_note" ? "note" : message.authorType}`}>
    ...
    {delivery && <span className={`delivery-badge ${message.deliveryStatus}`} title={message.deliveryError ?? undefined}>{delivery}</span>}
