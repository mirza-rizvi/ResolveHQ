...
export function QueueSidebar({ queue, counts, savedViews, onSelectQueue, onSaveView, onApplyView, onDeleteView }: QueueSidebarProps) {
  return <aside className="queue-panel">
    ...
    <nav aria-label="Inbox queues">
      ...
      >
        <Icon size={16} /><span>{label}</span>{counts && <b>{counts[key]}</b>}
      </button>)}
    </nav>
    ...
    <div className="saved-view-list">
      {savedViews.length
        ? savedViews.map((view) => <div key={view.id} className="saved-view-row">
          <button onClick={() => onApplyView(view)}><CircleDot size={12} />{view.name}</button>
          <button type="button" className="saved-view-remove" aria-label={`Delete ${view.name}`} onClick={() => onDeleteView(view.id)}><X size={12} /></button>
        </div>)
        : <p>No saved views</p>}
    </div>
