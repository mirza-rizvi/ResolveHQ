...
export function SettingsPage() {
  ...
  async function saveWorkspace(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      setError(errorMessage(reason, "Workspace could not be saved."));
    }
  }
  async function addInbox(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      form.reset();
      setMessage("Inbox added. Route this address to the ResolveHQ Worker in Cloudflare Email Routing.");
      await load();
    } catch (reason) {
    ...
    return (
      <div className="standard-page">
        {error ? (
          ...
          <div className="route-loading" aria-label="Loading settings" />
        )}
      </div>
    );
