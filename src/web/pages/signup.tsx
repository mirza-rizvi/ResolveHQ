...
export function SignupPage() {
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    const payload = Object.fromEntries([...form.entries()].filter(([, value]) => typeof value !== "string" || value.trim() !== ""));
    try {
      await api("/auth/signup", { method: "POST", body: JSON.stringify(payload) }); await refresh(); navigate("/inbox");
    } catch (reason) { setError(errorMessage(reason, "Workspace creation failed.")); }
  ...
  return <AuthSurface><div className="auth-heading"><h1>Start with a clean queue.</h1><p>Create your workspace and invite the team when you are ready.</p></div><form onSubmit={submit} className="auth-form">
    ...
    <label>Support email (optional)<Input name="supportEmail" type="email" placeholder="support@example.com" /><small>The address customers write to. You can add it later in Settings.</small></label>
