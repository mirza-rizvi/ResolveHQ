...
export function TeamPage() {
  const { session } = useAuth(); const toast = useToast(); const [members, setMembers] = useState<Member[]>([]); const [inviteUrl, setInviteUrl] = useState(""); const [error, setError] = useState("");
  const load = useCallback(async () => {
    try { setMembers((await api<{ members: Member[] }>("/organization/members")).members); }
    catch (reason) { toast.push(errorMessage(reason, "Teammates could not be loaded."), "error"); }
  }, [toast]);
