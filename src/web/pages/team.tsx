...
export function TeamPage() {
  ...
  async function updateMember(member: Member, values: { role?: "admin" | "agent"; disabled?: boolean }) { setError(""); try { await api(`/organization/members/${member.id}`, { method: "PATCH", body: JSON.stringify(values) }); await load(); } catch (reason) { setError(reason instanceof Error ? reason.message : "Team access could not be updated."); } }
