...
import { Building2, Mail, Plus, Search, TicketCheck, X } from "lucide-react";
  ...
  const load = useCallback(
    async (value = query) => {
      try {
        ...
        setLoaded(false);
        toast.push(errorMessage(reason, "Customers could not be loaded."), "error");
      }
    },
    [query, toast],
  );
  useEffect(() => {
    const timeout = window.setTimeout(() => void load(query), 200);
  ...
  return (
    <div className="standard-page">
      <header className="page-header">
        <div>
          <h1>Customers</h1>
          <p>People, companies, and the full history behind each conversation.</p>
        </div>
      ...
      <div className="customer-ledger">
        ...
        {loaded && !customers.length && (
          <p className="ledger-empty">
            {query
              ...
              : "No customers yet. They appear here as soon as an email arrives or you add one."}
          </p>
        )}
        {customers.map((customer) => (
          ...
          >
            ...
            <time>
              {customer.lastContactedAt ? new Date(customer.lastContactedAt).toLocaleDateString() : "No contact yet"}
            </time>
            <div>
              <TicketCheck size={14} />
      ...
      {detail && (
        <section className="customer-detail" aria-label={`${detail.customer.name} history`}>
          <header>
            <div>
            ...
            <button type="button" aria-label="Close customer details" onClick={() => setDetail(null)}>
              <X size={18} />
            </button>
          </header>
          <div className="customer-summary">
            ...
            <span>
              <strong>{detail.tickets.length}</strong>Total
            </span>
          </div>
          <form onSubmit={saveNotes}>
            <label>
              ...
              <textarea
                ...
                rows={3}
                placeholder="Context that helps the support team…"
              />
            </label>
          ...
          <div className="customer-history">
            ...
            {detail.tickets.length ? (
              detail.tickets.map((ticket) => (
                <Link key={ticket.id} to={`/inbox/${ticket.id}`}>
                  <span className={`status-pin status-${ticket.status}`} />
                  <div>
                    <strong>{ticket.subject}</strong>
                    <small>
                      #{ticket.number} · {ticket.status.replace("_", " ")}
                    </small>
                  </div>
