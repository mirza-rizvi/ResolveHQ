...
export function DashboardPage() {
  const { data, isPending, error } = useQuery({
    queryKey: ["dashboard"],
    queryFn: () => api<DashboardData>("/operations/dashboard"),
    refetchInterval: 30_000,
  });
  const { data: settings } = useQuery({
    queryKey: ["organization-settings"],
    queryFn: () => api<{ inboxes: unknown[] }>("/organization/settings"),
  });
  ...
  return (
    <div className="standard-page">
      <header className="page-header">
        <div>
          <h1>Overview</h1>
          <p>What needs the team’s attention right now.</p>
        </div>
        <Link className="text-link" to="/inbox">
          Open inbox <ArrowUpRight size={15} />
        </Link>
      </header>
      {settings && !settings.inboxes.length && (
        <p className="page-banner">
          <TriangleAlert size={15} />
          No support inbox configured — replies cannot be sent. <Link to="/settings">Add one in Settings.</Link>
        </p>
      )}
      <section className="measure-strip" aria-label="Ticket summary">
        {measures.map(([label, count, Icon, href]) => (
          <Link key={label} to={href} aria-busy={isPending}>
            <Icon size={17} />
            <span>{label}</span>
      ...
      <div className="dashboard-columns">
        ...
        <section>
          <div className="section-heading">
            ...
            <span>Team trail</span>
          </div>
          <div className="activity-ledger activity-events">
            {data?.recentActivity.map((event) =>
              event.ticketId ? (
                <Link key={event.id} to={`/inbox/${event.ticketId}`}>
...
function activityLabel(value: string) {
  const labels: Record<string, string> = {
    ...
    "ticket.agent_replied": "Agent replied",
    "ticket.note_added": "Private note added",
