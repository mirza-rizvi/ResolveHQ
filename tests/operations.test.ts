...
describe("operational workflows", () => {
  it("protects ticket updates with versions and isolates drafts", async () => {
    ...
    const customerResponse = await request(
      "/customers",
      { method: "POST", body: JSON.stringify({ name: "Version Customer", email: "version@example.test" }) },
      alpha,
    );
    ...
    const created = await request(
      ...
      {
        method: "POST",
        body: JSON.stringify({ customerId: customer.id, subject: "Concurrent update", message: "Initial message" }),
      },
      alpha,
    );
    ...
    expect(
      (
        await request(
          `/tickets/${ticket.id}`,
          { method: "PATCH", body: JSON.stringify({ priority: "high", version }) },
          alpha,
        )
      ).status,
    ).toBe(200);
    expect(
      (
        await request(
          `/tickets/${ticket.id}`,
          { method: "PATCH", body: JSON.stringify({ priority: "urgent", version }) },
          alpha,
        )
      ).status,
    ).toBe(409);
    ...
    expect(
      (
        await request(
          `/operations/tickets/${ticket.id}/draft`,
          { method: "PUT", body: JSON.stringify({ body: "Private draft", kind: "internal_note", revision: 0 }) },
          alpha,
        )
      ).status,
    ).toBe(200);
  ...
  it("returns exact aggregate metrics independently of ticket page size", async () => {
    ...
    const customerResponse = await request(
      "/customers",
      { method: "POST", body: JSON.stringify({ name: "Metrics Customer", email: "metrics@example.test" }) },
      workspace,
    );
    const customer = ((await customerResponse.json()) as { customer: { id: string } }).customer;
    for (let index = 0; index < 55; index += 1) {
      await request(
        ...
        {
          method: "POST",
          body: JSON.stringify({ customerId: customer.id, subject: `Metric ${index}`, message: "Measure this ticket" }),
        },
        workspace,
      );
    }
  ...
  it("isolates dev-mail captures by organization, lists system mail addressed to the caller, and blocks non-admin roles", async () => {
    ...
    await request(
      ...
      { method: "POST", body: JSON.stringify({ name: "Support", emailAddress: "alpha-support@example.test" }) },
      alpha,
    );
    const alphaCustomer = (
      (await (
        await request(
          "/customers",
          { method: "POST", body: JSON.stringify({ name: "Alpha Customer", email: "alpha-customer@example.test" }) },
          alpha,
        )
    ...
    const alphaTicket = (
      (await (
        await request(
          ...
          {
            method: "POST",
            body: JSON.stringify({ customerId: alphaCustomer.id, subject: "Alpha ticket", message: "Hello alpha" }),
          },
          alpha,
        )
    ...
    const alphaReply = (
      (await (
        await request(
          `/tickets/${alphaTicket.id}/messages`,
          { method: "POST", body: JSON.stringify({ body: "Alpha reply", kind: "message" }) },
          alpha,
        )
      ).json()) as { message: { id: string } }
    ).message;
    const alphaJob = await env.DB.prepare("SELECT id FROM outbound_mail_jobs WHERE message_id = ?")
      .bind(alphaReply.id)
      .first<{ id: string }>();
    ...
    await request(
      "/organization/inboxes",
      { method: "POST", body: JSON.stringify({ name: "Support", emailAddress: "beta-support@example.test" }) },
      beta,
    );
    const betaCustomer = (
      (await (
        await request(
          "/customers",
          { method: "POST", body: JSON.stringify({ name: "Beta Customer", email: "beta-customer@example.test" }) },
          beta,
        )
      ).json()) as { customer: { id: string } }
    ).customer;
    const betaTicket = (
      (await (
        await request(
          ...
          {
            method: "POST",
            body: JSON.stringify({ customerId: betaCustomer.id, subject: "Beta ticket", message: "Hello beta" }),
          },
          beta,
        )
      ).json()) as { ticket: { id: string } }
    ...
    const betaReply = (
      (await (
        await request(
          `/tickets/${betaTicket.id}/messages`,
          { method: "POST", body: JSON.stringify({ body: "Beta reply", kind: "message" }) },
          beta,
        )
      ).json()) as { message: { id: string } }
    ).message;
    const betaJob = await env.DB.prepare("SELECT id FROM outbound_mail_jobs WHERE message_id = ?")
      .bind(betaReply.id)
      .first<{ id: string }>();
    ...
    await sendSystemMail(env as AppBindings, {
      to: "owner-dev-mail-alpha@example.test",
      subject: "System notice for alpha",
      text: "hi",
    });
    ...
    const alphaCaptures = ((await alphaListing.json()) as { captures: Array<{ toAddress: string; subject: string }> })
      .captures;
    ...
    const inviteResponse = await request(
      ...
      { method: "POST", body: JSON.stringify({ email: "dev-mail-agent@example.test", role: "agent" }) },
      alpha,
    );
    ...
    const acceptResponse = await request("/auth/accept-invitation", {
      method: "POST",
      body: JSON.stringify({ token, name: "Agent Person", password: "a-secure-test-password" }),
    });
    ...
    const agentSession: TestSession = {
      cookie: found.map((match) => `${match[1]}=${match[2]}`).join("; "),
      csrf: acceptBody.csrfToken,
      userId: acceptBody.user.id,
      organizationId: acceptBody.organizationId,
    };
  ...
  it("returns 404 for GET /operations/dev-mail when capture mode is disabled or Resend is configured", async () => {
    ...
    const disabledResponse = await app.request(
      "http://localhost:8787/api/operations/dev-mail",
      { headers: { cookie: workspace.cookie } },
      { ...env, DEV_MAIL_MODE: "disabled" },
    );
    expect(disabledResponse.status).toBe(404);

    const resendConfiguredResponse = await app.request(
      "http://localhost:8787/api/operations/dev-mail",
      { headers: { cookie: workspace.cookie } },
  ...
  it("bulk updates refuse cross-tenant assignees and report skipped tickets", async () => {
    ...
    const customer = (
      (await (
        await request(
          "/customers",
          { method: "POST", body: JSON.stringify({ name: "B", email: "b@example.test" }) },
          alpha,
        )
      ).json()) as { customer: { id: string } }
    ).customer;
    const ticket = (
      (await (
        await request(
          "/tickets",
          { method: "POST", body: JSON.stringify({ customerId: customer.id, subject: "Bulk", message: "x" }) },
          alpha,
        )
      ).json()) as { ticket: { id: string } }
    ).ticket;
    const crossTenant = await request(
      ...
      { method: "POST", body: JSON.stringify({ ticketIds: [ticket.id], assignedUserId: beta.userId }) },
      alpha,
    );
    ...
    const result = (await (
      await request(
        "/operations/tickets/bulk",
        { method: "POST", body: JSON.stringify({ ticketIds: [ticket.id, "tkt_missing"], status: "resolved" }) },
        alpha,
      )
    ).json()) as { updated: number; skipped: Array<{ ticketId: string }> };
    expect(result.updated).toBe(1);
    expect(result.skipped).toEqual([{ ticketId: "tkt_missing", reason: "ticket_not_found" }]);
    const row = await env.DB.prepare("SELECT resolved_at AS resolvedAt, version FROM tickets WHERE id = ?")
      .bind(ticket.id)
      .first<{ resolvedAt: number | null; version: number }>();
  ...
  it("rejects a bulk update for an unknown team before writing any ticket", async () => {
    ...
    const customer = (
      (await (
        await request(
          "/customers",
          { method: "POST", body: JSON.stringify({ name: "T", email: "t@example.test" }) },
          workspace,
        )
      ).json()) as { customer: { id: string } }
    ).customer;
    const ticket = (
      (await (
        await request(
          "/tickets",
          { method: "POST", body: JSON.stringify({ customerId: customer.id, subject: "Team bulk", message: "x" }) },
          workspace,
        )
      ).json()) as { ticket: { id: string } }
    ).ticket;
    const before = await env.DB.prepare("SELECT version FROM tickets WHERE id = ?")
      .bind(ticket.id)
      .first<{ version: number }>();
    const response = await request(
      "/operations/tickets/bulk",
      { method: "POST", body: JSON.stringify({ ticketIds: [ticket.id], assignedTeamId: "tem_missing" }) },
      workspace,
    );
    expect(response.status).toBe(404);
    expect(((await response.json()) as { error: { code: string } }).error.code).toBe("team_not_found");
    const after = await env.DB.prepare("SELECT version FROM tickets WHERE id = ?")
      .bind(ticket.id)
      .first<{ version: number }>();
  ...
  it("reports queue counts and de-duplicates saved views", async () => {
    ...
    const customer = (
      (await (
        await request(
          "/customers",
          { method: "POST", body: JSON.stringify({ name: "C", email: "c@example.test" }) },
          workspace,
        )
      ).json()) as { customer: { id: string } }
    ).customer;
    await request(
      "/tickets",
      { method: "POST", body: JSON.stringify({ customerId: customer.id, subject: "One", message: "x" }) },
      workspace,
    );
    const counts = (
      (await (await request("/tickets/counts", {}, workspace)).json()) as { counts: Record<string, number> }
    ).counts;
    expect(counts).toMatchObject({ all: 1, waiting_customer: 1, open: 0, unassigned: 1, mine: 0 });
    const first = await request(
      "/operations/views",
      { method: "POST", body: JSON.stringify({ name: "Open tickets", filters: { status: "open" } }) },
      workspace,
    );
    ...
    const views = (
      (await (await request("/operations/views", {}, workspace)).json()) as { views: Array<{ id: string }> }
    ).views;
