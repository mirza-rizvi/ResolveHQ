...
describe("mail queue workflow", () => {
  it("creates an isolated ticket from inbound email and de-duplicates provider messages", async () => {
    ...
    await env.DB.prepare("UPDATE organizations SET support_email = ? WHERE id = ?")
      .bind("help-alpha@example.test", alpha.organizationId)
      .run();
    await env.DB.prepare("UPDATE organizations SET support_email = ? WHERE id = ?")
      .bind("help-beta@example.test", beta.organizationId)
      .run();
    const raw = mimeMessage({
      id: "<inbound-1@example.test>",
      to: "help-alpha@example.test",
    ...
    const alphaTickets = await env.DB.prepare("SELECT count(*) AS count FROM tickets WHERE organization_id = ?")
      .bind(alpha.organizationId)
      .first<{ count: number }>();
    const betaTickets = await env.DB.prepare("SELECT count(*) AS count FROM tickets WHERE organization_id = ?")
      .bind(beta.organizationId)
      .first<{ count: number }>();
    const messages = await env.DB.prepare(
      "SELECT count(*) AS count FROM messages WHERE organization_id = ? AND provider_message_id = ?",
    )
      .bind(alpha.organizationId, "<inbound-1@example.test>")
      .first<{ count: number }>();
  ...
  it("stages mail larger than the queue limit and cleans up the R2 object", async () => {
    ...
    await env.DB.prepare(
      "INSERT INTO inbound_mail_events (id, staging_object_key, status, attachment_cursor, attempts, created_at, updated_at) VALUES (?, ?, 'staged', 0, 0, ?, ?)",
    )
      .bind(eventId, stagingObjectKey, now, now)
      .run();
  ...
  it("does not attach a forged subject ticket number to another customer", async () => {
    ...
    await env.DB.prepare("UPDATE organizations SET support_email = ? WHERE id = ?")
      .bind("forged@example.test", workspace.organizationId)
      .run();
    const customerResponse = await import("./helpers").then(({ request }) =>
      request(
        "/customers",
        { method: "POST", body: JSON.stringify({ name: "Victim", email: "victim@example.test" }) },
        workspace,
      ),
    );
    const customer = ((await customerResponse.json()) as { customer: { id: string } }).customer;
    const ticketResponse = await import("./helpers").then(({ request }) =>
      request(
        "/tickets",
        {
          method: "POST",
    ...
    await processInboundMail(env as AppBindings, {
      raw: mimeMessage({
        id: "<forged@example.test>",
        to: "forged@example.test",
        subject: `Re: [#${ticket.number}] Private billing issue`,
        body: "Please expose the history.",
      }),
      from: "customer@example.test",
      to: "forged@example.test",
    });

    const count = await env.DB.prepare("SELECT count(*) AS count FROM tickets WHERE organization_id = ?")
  ...
  it("captures outbound mail in development mode", async () => {
    ...
    await request(
      ...
      { method: "POST", body: JSON.stringify({ name: "Support", emailAddress: "capture@example.test" }) },
      workspace,
    );
    const customer = (
      (await (
        await request(
          "/customers",
          { method: "POST", body: JSON.stringify({ name: "Cap", email: "cap@example.test" }) },
          workspace,
        )
    ...
    const ticket = (
      (await (
        await request(
          ...
          {
            method: "POST",
            body: JSON.stringify({ customerId: customer.id, subject: "Capture me", message: "Hello" }),
          },
          workspace,
        )
    ...
    const reply = (await (
      await request(
        ...
        { method: "POST", body: JSON.stringify({ body: "Reply body", kind: "message" }) },
        workspace,
      )
    ...
    const capture = await env.DB.prepare(
      'SELECT to_address AS "to", subject FROM mail_captures WHERE organization_id = ? ORDER BY created_at DESC LIMIT 1',
    )
      .bind(workspace.organizationId)
      .first<{ to: string; subject: string }>();
  ...
  it("threads a customer reply onto the ticket the agent replied from", async () => {
    ...
    await request(
      "/organization/inboxes",
      { method: "POST", body: JSON.stringify({ name: "Support", emailAddress: "thread@example.test" }) },
      workspace,
    );
    await processInboundMail(env as AppBindings, {
      raw: mimeMessage({
        id: "<first@example.test>",
        to: "thread@example.test",
    ...
    const ticket = await env.DB.prepare("SELECT id FROM tickets WHERE organization_id = ?")
      .bind(workspace.organizationId)
      .first<{ id: string }>();
    const reply = (await (
      await request(
        `/tickets/${ticket!.id}/messages`,
        { method: "POST", body: JSON.stringify({ body: "Try resetting.", kind: "message" }) },
        workspace,
      )
    ).json()) as { message: { id: string } };
    const job = await env.DB.prepare("SELECT id FROM outbound_mail_jobs WHERE message_id = ?")
      ...
      .first<{ id: string }>();
    await processOutboundMail(env as AppBindings, { jobId: job!.id });
    const sent = await env.DB.prepare("SELECT rfc_message_id AS rfc FROM messages WHERE id = ?")
      .bind(reply.message.id)
      .first<{ rfc: string }>();
    ...
    const state = await env.DB.prepare("SELECT status, message_count AS messages FROM tickets WHERE id = ?")
      .bind(ticket!.id)
      .first<{ status: string; messages: number }>();
  ...
  it("threads a reply that carries only a multi-id References header", async () => {
    ...
    await request(
      "/organization/inboxes",
      { method: "POST", body: JSON.stringify({ name: "Support", emailAddress: "references@example.test" }) },
      workspace,
    );
    await processInboundMail(env as AppBindings, {
      raw: mimeMessage({
    ...
    const reply = (await (
      await request(
        `/tickets/${ticket!.id}/messages`,
        { method: "POST", body: JSON.stringify({ body: "Looking into it.", kind: "message" }) },
        workspace,
      )
    ).json()) as { message: { id: string } };
    const job = await env.DB.prepare("SELECT id FROM outbound_mail_jobs WHERE message_id = ?")
    ...
    await processInboundMail(env as AppBindings, {
      raw: mimeMessage({
        id: "<r2@example.test>",
        to: "references@example.test",
        subject: "Re: Export fails",
        body: "Still failing.",
        references: `<junk-a@elsewhere.test> <junk-b@elsewhere.test> ${sent!.rfc}`,
      }),
      from: "customer@example.test",
      to: "references@example.test",
    });
  ...
  it("fails the job and the message when no support address is configured", async () => {
    ...
    const customer = (
      (await (
        await request(
          "/customers",
          { method: "POST", body: JSON.stringify({ name: "Nina", email: "nina@example.test" }) },
          workspace,
        )
      ).json()) as { customer: { id: string } }
    ).customer;
    const ticket = (
      (await (
        await request(
          ...
          {
            method: "POST",
            body: JSON.stringify({ customerId: customer.id, subject: "No inbox", message: "Original request" }),
          },
          workspace,
        )
      ).json()) as { ticket: { id: string } }
    ...
    const jobState = await env.DB.prepare("SELECT status, last_error AS error FROM outbound_mail_jobs WHERE id = ?")
      .bind(job!.id)
  ...
  it("falls back to the subject ticket number only for the same customer", async () => {
    ...
    await request(
      "/organization/inboxes",
      { method: "POST", body: JSON.stringify({ name: "Support", emailAddress: "subject@example.test" }) },
      workspace,
    );
    await processInboundMail(env as AppBindings, {
      raw: mimeMessage({
    ...
    const ticket = await env.DB.prepare("SELECT number FROM tickets WHERE organization_id = ?")
      .bind(workspace.organizationId)
      .first<{ number: number }>();
    await processInboundMail(env as AppBindings, {
      raw: mimeMessage({
        id: "<s2@example.test>",
        to: "subject@example.test",
        subject: `Re: [#${ticket!.number}] Invoice`,
        body: "Following up.",
      }),
      from: "customer@example.test",
      to: "subject@example.test",
...
describe("system mail", () => {
  it("persists system mail as an organization-less capture using the default from address", async () => {
    await sendSystemMail(env as AppBindings, {
      to: "notify-default@example.test",
      subject: "Default from address",
      text: "hello",
    });
    const capture = await env.DB.prepare(
      'SELECT organization_id AS organizationId, from_address AS "from" FROM mail_captures WHERE to_address = ?',
    )
      .bind("notify-default@example.test")
      .first<{ organizationId: string | null; from: string }>();
  ...
  it("honors a SYSTEM_MAIL_FROM override", async () => {
    await sendSystemMail({ ...env, SYSTEM_MAIL_FROM: "alerts@resolvehq.test" } as AppBindings, {
      to: "notify-override@example.test",
      subject: "Custom from address",
      text: "hello",
    });
    const capture = await env.DB.prepare('SELECT from_address AS "from" FROM mail_captures WHERE to_address = ?')
      .bind("notify-override@example.test")
      .first<{ from: string }>();
  ...
  it("rejects when no outgoing mail provider is configured", async () => {
    await expect(
      sendSystemMail({ ...env, DEV_MAIL_MODE: "disabled" } as AppBindings, {
        to: "notify-unconfigured@example.test",
        subject: "No provider",
        text: "hello",
      }),
...
describe("scheduled mail recovery", () => {
  it("recovers stalled jobs, re-enqueues staged inbound email, and clears expired staging objects", async () => {
    ...
    await env.DB.prepare("UPDATE organizations SET support_email = ? WHERE id = ?")
      .bind("cron@example.test", workspace.organizationId)
      .run();
    const { request } = await import("./helpers");
    const customer = (
      (await (
        await request(
          "/customers",
          { method: "POST", body: JSON.stringify({ name: "Cron", email: "cron-customer@example.test" }) },
          workspace,
        )
      ).json()) as { customer: { id: string } }
    ).customer;
    await request(
      ...
      {
        method: "POST",
        body: JSON.stringify({ customerId: customer.id, subject: "Stalled send", message: "Please reply." }),
      },
      workspace,
    );
    const stale = Date.now() - 30 * 60 * 1000;
    await env.DB.prepare(
      "UPDATE outbound_mail_jobs SET status = 'processing', updated_at = ? WHERE organization_id = ?",
    )
      .bind(stale, workspace.organizationId)
      .run();
    ...
    await env.ATTACHMENTS.put(
      stagingObjectKey,
      mimeMessage({
        id: "<cron@example.test>",
        to: "cron@example.test",
        subject: "Stalled inbound",
        body: "Recover me.",
      }),
    );
    await env.DB.prepare(
      "INSERT INTO inbound_mail_events (id, staging_object_key, status, attachment_cursor, attempts, created_at, updated_at) VALUES (?, ?, 'processing', 0, 1, ?, ?)",
    )
      .bind(eventId, stagingObjectKey, stale, stale)
      .run();
    ...
    await env.DB.prepare(
      "INSERT INTO inbound_mail_events (id, staging_object_key, status, attachment_cursor, attempts, created_at, updated_at) VALUES (?, ?, 'completed', 0, 1, ?, ?)",
    )
      .bind("ime_cron_expired", expiredKey, 0, Date.now() - 30 * 24 * 60 * 60 * 1000)
      .run();
    ...
    const recovered = await env.DB.prepare("SELECT status FROM inbound_mail_events WHERE id = ?")
      .bind(eventId)
      .first<{ status: string }>();
    expect(recovered?.status).toBe("failed");
    expect(await env.ATTACHMENTS.head(expiredKey)).toBeNull();
    const retired = await env.DB.prepare("SELECT staging_object_key AS key FROM inbound_mail_events WHERE id = ?")
      .bind("ime_cron_expired")
      .first<{ key: string }>();
    expect(retired?.key).toBe("deleted/ime_cron_expired");
    expect(await env.ATTACHMENTS.head(stagingObjectKey)).not.toBeNull();
    const live = await env.DB.prepare("SELECT staging_object_key AS key FROM inbound_mail_events WHERE id = ?")
      .bind(eventId)
      .first<{ key: string }>();
    ...
    const completed = await env.DB.prepare("SELECT status FROM inbound_mail_events WHERE id = ?")
      .bind(eventId)
      .first<{ status: string }>();
    expect(completed?.status).toBe("completed");
    const ticket = await env.DB.prepare("SELECT subject FROM tickets WHERE organization_id = ? AND subject = ?")
      .bind(workspace.organizationId, "Stalled inbound")
  ...
  it("adopts an agent reply whose outbound job never landed", async () => {
    ...
    const customer = (
      (await (
        await request(
          "/customers",
          { method: "POST", body: JSON.stringify({ name: "Orphan", email: "orphan@example.test" }) },
          workspace,
        )
      ).json()) as { customer: { id: string } }
    ).customer;
    const ticket = (
      (await (
        await request(
          "/tickets",
          { method: "POST", body: JSON.stringify({ customerId: customer.id, subject: "Orphan", message: "first" }) },
          workspace,
        )
      ).json()) as { ticket: { id: string } }
    ).ticket;
    ...
    await env.DB.prepare(
      "INSERT INTO messages (id, organization_id, ticket_id, author_type, author_user_id, kind, body_text, normalized_search, delivery_status, created_at) VALUES (?, ?, ?, 'agent', ?, 'message', 'orphaned reply', 'orphaned reply', 'queued', ?)",
    )
      .bind(messageId, workspace.organizationId, ticket.id, workspace.userId, Date.now() - 5 * 60 * 1000)
      .run();
    expect(
      await env.DB.prepare("SELECT id FROM outbound_mail_jobs WHERE message_id = ?").bind(messageId).first(),
    ).toBeNull();
    ...
    const job = await env.DB.prepare(
      ...
      .bind(messageId)
      .first<{ id: string; status: string; idempotencyKey: string }>();
  ...
  it("counts an attempt and records the reason when the staged payload is gone", async () => {
    ...
    const now = Date.now();
    await env.DB.prepare(
      "INSERT INTO inbound_mail_events (id, staging_object_key, status, attachment_cursor, attempts, created_at, updated_at) VALUES (?, ?, 'failed', 0, 0, ?, ?)",
    )
      .bind(eventId, stagingObjectKey, now, now)
      .run();
    ...
    const row = await env.DB.prepare(
      ...
      .bind(eventId)
      .first<{ attempts: number; status: string; lastError: string }>();
