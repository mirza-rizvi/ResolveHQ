...
describe("attachment authorization", () => {
  it("uploads before sending and links attachments to the new message", async () => {
    ...
    await request(
      ...
      { method: "POST", body: JSON.stringify({ name: "Support", emailAddress: "attach-first@example.test" }) },
      workspace,
    );
    const customer = (
      (await (
        await request(
          "/customers",
          { method: "POST", body: JSON.stringify({ name: "A", email: "a@example.test" }) },
          workspace,
        )
    ...
    const ticket = (
      (await (
        await request(
          "/tickets",
          { method: "POST", body: JSON.stringify({ customerId: customer.id, subject: "Attach", message: "x" }) },
          workspace,
        )
    ...
    ).json()) as { upload: { attachmentId: string; url: string } };
    const upload = await request(
      intent.upload.url.replace(/^\/api/, ""),
      {
        method: "PUT",
        body: file,
        headers: { "content-type": "text/plain", "content-length": String(file.byteLength) },
      },
      workspace,
    );
    ...
    const message = (await (
      await request(
        ...
        {
          method: "POST",
          body: JSON.stringify({ body: "see log", kind: "message", attachmentIds: [intent.upload.attachmentId] }),
        },
        workspace,
      )
    ).json()) as { message: { id: string } };
    const row = await env.DB.prepare("SELECT message_id AS messageId FROM attachments WHERE id = ?")
      .bind(intent.upload.attachmentId)
    ...
    expect(
      (
        await request(
          ...
          {
            method: "POST",
            body: JSON.stringify({ body: "again", kind: "message", attachmentIds: [intent.upload.attachmentId] }),
          },
          workspace,
        )
      ).status,
  ...
  it("refuses to create a message when an attachment belongs to another uploader or ticket", async () => {
    ...
    const first = (
      (await (
        await request(
          "/tickets",
          { method: "POST", body: JSON.stringify({ customerId: customer.id, subject: "First", message: "x" }) },
          workspace,
        )
      ).json()) as { ticket: { id: string } }
    ).ticket;
    const second = (
      (await (
        await request(
          "/tickets",
          { method: "POST", body: JSON.stringify({ customerId: customer.id, subject: "Second", message: "y" }) },
          workspace,
        )
      ).json()) as { ticket: { id: string } }
    ).ticket;
    ...
    // message row exists.
    const wrongTicket = await request(
      `/tickets/${second.id}/messages`,
      {
        method: "POST",
        body: JSON.stringify({ body: "wrong ticket", kind: "message", attachmentIds: [intent.upload.attachmentId] }),
      },
      workspace,
    );
    ...
    const stray = await env.DB.prepare(
      "SELECT count(*) AS total FROM messages WHERE ticket_id = ? AND body_text = 'wrong ticket'",
    )
      .bind(second.id)
      .first<{ total: number }>();
    ...
    const missing = await request(
      `/tickets/${first.id}/messages`,
      {
        method: "POST",
        body: JSON.stringify({
          body: "missing",
          kind: "message",
          attachmentIds: [intent.upload.attachmentId, "att_does_not_exist"],
        }),
      },
      workspace,
    );
    expect(missing.status).toBe(404);
    const unlinked = await env.DB.prepare("SELECT message_id AS messageId FROM attachments WHERE id = ?")
      .bind(intent.upload.attachmentId)
      .first<{ messageId: string | null }>();
  ...
  it("lets exactly one of two concurrent sends claim the same attachment", async () => {
    ...
    const customer = (
      (await (
        await request(
          "/customers",
          { method: "POST", body: JSON.stringify({ name: "D", email: "d@example.test" }) },
          workspace,
        )
      ).json()) as { customer: { id: string } }
    ).customer;
    const ticket = (
      (await (
        await request(
          "/tickets",
          { method: "POST", body: JSON.stringify({ customerId: customer.id, subject: "Race", message: "x" }) },
          workspace,
        )
      ).json()) as { ticket: { id: string } }
    ).ticket;
    ...
    const send = (clientMessageId: string) =>
      request(
        `/tickets/${ticket.id}/messages`,
        {
          method: "POST",
          body: JSON.stringify({
            body: `race ${clientMessageId}`,
            kind: "message",
            clientMessageId,
            attachmentIds: [intent.upload.attachmentId],
          }),
        },
        workspace,
      );
    ...
    const kept = await env.DB.prepare(
      "SELECT client_message_id AS clientMessageId FROM messages WHERE ticket_id = ? AND client_message_id IS NOT NULL",
    )
      .bind(ticket.id)
      .all<{ clientMessageId: string }>();
    expect(kept.results.map((entry) => entry.clientMessageId)).toEqual([winnerClientId]);
    expect(
      await env.DB.prepare("SELECT id FROM messages WHERE client_message_id = ?").bind(loserClientId).first(),
    ).toBeNull();
  });
  ...
  it("sweeps orphaned uploads and their objects after a day", async () => {
    ...
    const ticket = (
      (await (
        await request(
          "/tickets",
          { method: "POST", body: JSON.stringify({ customerId: customer.id, subject: "Orphan", message: "x" }) },
          workspace,
        )
      ).json()) as { ticket: { id: string } }
    ).ticket;
    ...
    await env.DB.prepare(
      "INSERT INTO attachments (id, organization_id, ticket_id, message_id, object_key, filename, content_type, size, checksum, uploaded_by_user_id, created_at) VALUES (?, ?, ?, NULL, ?, 'orphan.txt', 'text/plain', 16, 'sum', ?, ?)",
    )
      .bind(
        "att_orphan_fixture",
    ...
    await env.DB.prepare(
      "INSERT INTO attachments (id, organization_id, ticket_id, message_id, object_key, filename, content_type, size, checksum, uploaded_by_user_id, created_at) VALUES (?, ?, ?, NULL, ?, 'fresh.txt', 'text/plain', 13, 'sum', ?, ?)",
    )
      .bind("att_fresh_fixture", workspace.organizationId, ticket.id, fresh, workspace.userId, Date.now())
      .run();
    ...
    expect(
      await env.DB.prepare("SELECT id FROM attachments WHERE id = ?").bind("att_fresh_fixture").first(),
    ).not.toBeNull();
